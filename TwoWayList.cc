...
template <class Type> void
TwoWayList <Type> :: Copy(const TwoWayList &copyme)
{
	Destroy();
	...
	int leftSize = list->leftSize = copyme.list->leftSize;
	int rightSize = list->rightSize = copyme.list->rightSize;

	Node *temp = new (std::nothrow) Node[leftSize + rightSize + 2];
	if (temp == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	...
	list->first = &temp[0];
	temp[0].next = &temp[1];
	list->last = &temp[leftSize + rightSize + 1];
	temp[leftSize + rightSize + 1].previous = &temp[leftSize + rightSize];
	list->current = &temp[leftSize];

	Node *iter = (copyme.list)->first->next;
	for (int i = 1; i <= list->leftSize + list->rightSize; i++) {
		temp[i].next = &temp[i+1];
		temp[i].previous = &temp[i-1];
		temp[i].data = new (std::nothrow) Type;
		(temp[i].data)->Copy(iter->data);
		iter = iter->next;
	} 
}

template <class Type> void
TwoWayList <Type> :: Destroy(){
	MoveToStart ();
	while (RightLength ()>0) {
		Type temp;
		Remove (&temp);
